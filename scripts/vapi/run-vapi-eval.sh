...
#
# Usage:
#   ./scripts/vapi/run-vapi-eval.sh                    # Run all evals
#   ./scripts/vapi/run-vapi-eval.sh --test "Booking"   # Run matching test(s)
# ─────────────────────────────────────────────────────────────
...
EVALS_FILE="${ROOT_DIR}/vapi/evals-test-suite.json"
...
check_backend() {
    ...
    local webhook_url="${backend_url}/functions/v1/webhook"

    local http_code
    http_code=$(curl -s -o /dev/null -w "%{http_code}" "${webhook_url}" 2>/dev/null || echo "000")

    if [ "${http_code}" = "200" ]; then
        echo -e "  ${GREEN}✅ Backend reachable (${webhook_url})${NC}"
    else
        echo -e "  ${RED}❌ Backend unreachable (HTTP ${http_code})${NC}"
        echo -e "  ${YELLOW}⚠️  Evals will fail without a working backend${NC}"
        echo ""
        echo "  Make sure the Supabase webhook runtime is running on the host:"
        echo "    ./scripts/run-vapi-local.sh"
        echo "    curl -sS https://<your-tunnel-host>/functions/v1/webhook"
        echo ""
