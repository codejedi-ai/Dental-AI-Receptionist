...
#
# Usage:
#   ./scripts/vapi/deploy-vapi.sh              # Deploy assistant config
#   ./scripts/vapi/deploy-vapi.sh --evals      # Deploy config + analysis plan (evals)
#   ./scripts/vapi/deploy-vapi.sh --dry-run    # Show diff without deploying
# ─────────────────────────────────────────────────────────────
...
ASSISTANT_CONFIG="${VAPI_DIR}/riley-assistant.json"
ANALYSIS_CONFIG="${VAPI_DIR}/analysis-plan.json"
...
import json
from pathlib import Path
p = Path("${ROOT_DIR}") / "vapi" / "riley-assistant.json"
try:
...
PY
    ...
    local health_url="${server_url}"
    echo -e "  🔗 Assistant serverUrl: ${server_url}"
    echo -e "  🩺 Webhook readiness: ${health_url}"
    ...
    if [ "${http_code}" = "200" ]; then
        ...
        echo "  Response: ${body}"
        echo "  Check: ./scripts/run-vapi-local.sh running, tunnel up, then curl your public /functions/v1/webhook URL"
        exit 1
    fi
}
