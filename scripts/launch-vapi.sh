...
# Docker is used only for local data services. Vapi webhook runtime is Supabase Edge Function via ./scripts/run-vapi-local.sh
# Seeds Excel → Postgres via local Python (./scripts/seed-schedule-local.sh), not a Docker image.
...
if ! "$ROOT/scripts/seed-schedule-local.sh" "$EXCEL"; then
...
info "Start Vapi on the host:  $ROOT/scripts/run-vapi-local.sh"
info "Tunnel on this PC: ./scripts/tailscale-expose-vapi.sh — PUBLIC_BASE_URL + Vapi serverUrl https://.../functions/v1/webhook"
