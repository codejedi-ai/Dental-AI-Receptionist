...
interface TranscriptMessage { role: "assistant" | "user"; text: string; }
...
export default function VoiceWidget() {
  ...
  const startCall = async () => {
    if (!publicKey || !assistantId) {
      setTranscript([{ role: "assistant", text: "Voice assistant is not configured. Please set VAPI keys in environment variables." }]);
      setIsOpen(true);
      return;
    }
    try {
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-4 sm:right-6 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-gray-200 z-50 overflow-hidden">
          ...
          <div className="p-3 bg-white border-t border-gray-100 flex justify-center">
            {!isActive ? (
              <button onClick={startCall} disabled={isConnecting} className="bg-teal-600 hover:bg-teal-700 disabled:bg-teal-400 text-white px-6 py-2 rounded-full text-sm font-medium transition-colors flex items-center space-x-2">
                <span>🎤</span><span>{isConnecting ? "Connecting..." : "Start Talking"}</span>
              </button>
            ) : (
              <button onClick={endCall} className="bg-red-500 hover:bg-red-600 text-white px-6 py-2 rounded-full text-sm font-medium transition-colors flex items-center space-x-2">
                <span>⏹️</span><span>End Call</span>
              </button>
            )}
          </div>
        </div>
